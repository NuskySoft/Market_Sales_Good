...
fun MenuHamburguesa(
    ...
    // Versión V10 (localizado)
    val vtext = ConfigurationManager.versionAppValue
    val versionText = if (esPremium) {
        "${StringResourceManager.getString("premium", currentLanguage)} $vtext"
        } else {
        "${StringResourceManager.getString("free", currentLanguage)} $vtext"
        }
            ...
            Box(
                ...
                Column(
                    ...
                    if (isAuthenticated && currentUser != null) {
                        Text(
                            text = "👤 " + (usuarioEmail ?: displayName
                            ?: StringResourceManager.getString("usuario", currentLanguage)),
                            fontSize = 12.sp,
                            color = MaterialTheme.colorScheme.onPrimary.copy(alpha = 0.7f)
                        )
                    } else {
                        Text(
                            text = "👤 " + StringResourceManager.getString("usuario_invitado", currentLanguage),
                            fontSize = 12.sp,
                            color = MaterialTheme.colorScheme.onPrimary.copy(alpha = 0.7f)
                        )
                    }
...
//            }
                ...
                item {
                    MenuOption(
                        ...
                        onClick = {
                            scope.launch { drawerState.close() }
                            // Abre la pantalla de utilidades; si el usuario no es premium
                            // las opciones se mostrarán deshabilitadas con un mensaje informativo
                ...
                // Opciones de depuración solo en builds DEBUG
                if (BuildConfig.DEBUG) {
                    item {
                        MenuOption(
                            ...
                            onClick = {
                                scope.launch { drawerState.close() }
                                navController.navigate("debug_datos")
                            }
                        )
                    }
                }
...
//        // Versión V10
//        val vtext = ConfigurationManager.versionAppValue
//       val versionText = if (esPremium) "Premium $vtext" else "Free $vtext"
//        //val versionText = ConfigurationManager.versionAppValue
//
//        ModalDrawerSheet(
//            modifier = Modifier.width(280.dp),
//            drawerContainerColor = MaterialTheme.colorScheme.surface
